...
static int WriteHashTable(FILE* f, IndexFileOffset_t offset, HashTable* ht,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    ...
    record_pos += check;
    check = WriteHTBucket(f, bucket_pos, list, fn);
    if (check == kFailedWrite) {
      return kFailedWrite;
    }
    bucket_pos += check;
...
static int WriteHTBucketRecord(FILE* f, IndexFileOffset_t offset,
  ...
  BucketRecord header(num_elts, bucket_offset);
  header.ToDiskFormat();
  ...
  if (fwrite(&header, sizeof(BucketRecord), 1, f) != 1) {
    return kFailedWrite;
  }
...
static int WriteHTBucket(FILE* f, IndexFileOffset_t offset, LinkedList* li,
  ...
  for (int i = 0; i < num_elts; i++) {
    ...
    ElementPositionRecord header(element_pos);
    header.ToDiskFormat();
    int check = fwrite(&header, sizeof(ElementPositionRecord), 1, f);
    if (check == kFailedWrite) {
      return kFailedWrite;
    }
    ...
    check = fn(f, element_pos, payload);
    if (check == kFailedWrite) {
      return kFailedWrite;
    }
...
static int WriteDocidToDocnameFn(FILE* f, IndexFileOffset_t offset,
  ...
  int check = fwrite(kv->value, sizeof(char), file_name_bytes, f);
  if (check == kFailedWrite) {
    return kFailedWrite;
  }
...
static int WriteDocIDToPositionListFn(FILE* f,
  ...
  for (int i = 0; i < num_positions; i++) {
    ...
    if (fwrite(&position, sizeof(DocPositionOffset_t), 1, f) != 1) {
      return kFailedWrite;
    }
...
static int WriteWordToPostingsFn(FILE* f,
  ...
  // place in the file.
  int check = fwrite(wp->word, sizeof(char), word_bytes, f);
  if (check == kFailedWrite) {
    return kFailedWrite;
  }
