...
bool DocIDTableReader::LookupDocID(
  ...
  for (IndexFileOffset_t& curr_element : elements) {
    ...
    Verify333(check != -1);
    check = fread(&curr_header, sizeof(DocIDElementHeader), 1, file_);
    Verify333(check == 1);
    ...
    if (curr_header.doc_id == doc_id) {
      ...
      int positions_left = curr_header.num_positions;
      while (positions_left > 0) {
        u_int32_t pos;
        check = fread(&pos, sizeof(u_int32_t), 1, file_);
        Verify333(check == 1);
        pos = ntohl(pos);
        ret_val->push_front(pos);
        positions_left--;
      }
...
list<DocIDElementHeader> DocIDTableReader::GetDocIDList() const {
  ...
  for (int i = 0; i < header_.num_buckets; i++) {
    ...
    for (int j = 0; j < bucket_rec.chain_num_elements; j++) {
      ...
      check = fread(&element_pos, sizeof(ElementPositionRecord), 1, file_);
      Verify333(check == 1);
      ...
      check = fseek(file_, element_pos.position, SEEK_SET);
      Verify333(check != -1);
      check = fread(&element, sizeof(DocIDElementHeader), 1, file_);
